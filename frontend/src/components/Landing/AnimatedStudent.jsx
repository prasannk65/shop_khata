...
const AnimatedStudent = () => {
  ...
  useEffect(() => {
    ...
    // Select elements
    const studentWhole = s.select('#stud_whole');
    const head = s.select('#stud_head');
    const eyes = s.selectAll('.eye');
    const reflection = s.selectAll('.reflection');
    const hair = s.select('#hair');
    const dashed = s.select('#dashed');
    const stars = s.selectAll('.stars');
    const circle = s.selectAll('.circle');
    const pageFold = s.select('#pgFld');
    ...
    const mina = window.mina || Snap.mina;

    function bookIn() {
      if (!mina) {
        console.warn("Snap mina engine not found, skipping book animation");
        return;
      }

      // Reset book attributes to starting state to ensure clean animation
      bottomPages.attr({ points: "199.827,333.441 200.188,334 198.801,334 198.14,333.344 190.077,333.696 190,333 209,333 208.487,333.463" });

      bottomPages.animate({ points: "199,174 200,305 200,304 199,174 190,174 190,333 209,333 209,174" }, 350, mina.easeInOut, function () {
        setTimeout(function () {
          bottomPages.animate({ points: "313.061,304.975 200.188,305 198.801,305 85.44,304.975 30.5,333 190,333 209,333 370,333" }, 800, mina.bounce);
        }, 100)
      });

      bottomEdge.animate({ d: "M184.5,169l0.5,172.5c0,0,5.635,7,14.661,7s14.839-7,14.839-7V169" }, 350, mina.easeInOut, function () {
        setTimeout(function () {
          bottomEdge.animate({ d: "M13,341.5h172.224c0,0,5.973,7,15,7s14.776-7,14.776-7h172" }, 800, mina.bounce);
        }, 100)
      });
      ...
      bottomBody.animate({ d: "M205.94,174L200,307l-7.524-133l-8.602,0.26L184.512,338 c0,0,0.044,0.473,0.138,0.637c0.779,1.371,4.964,7.938,15.35,7.938c10.182,0,14.449-6.029,15.356-7.561 c0.13-0.219,0.191-0.299,0.191-0.299L216.212,174H205.94z" }, 350, mina.easeInOut, function () {
        setTimeout(function () {
          bottomBody.animate({ d: "M328.178,308.28L200,307.238L72.591,308.28 l-60.046,30.913l171.995-0.196c0,0,0.016,0.49,0.11,0.654c0.779,1.371,4.964,6.932,15.35,6.932 c10.182,0,14.449-6.033,15.356-7.564c0.13-0.219,0.191-0.063,0.191-0.063L387.241,339L328.178,308.28z" }, 800, mina.bounce);
        }, 100)
      });

      bottomRight.animate({ points: "199,299 199.016,174 199.7,174 199,326" }, 350, mina.easeInOut, function () {
        setTimeout(function () {
          bottomRight.animate({ points: "199,299 300.647,299 355.376,326 199,326" }, 800, mina.bounce);
        }, 100)
      });
      ...
      bottomLeft.animate({ points: "199,299 199.016,174 198.359,173.984 199,326" }, 350, mina.easeInOut, function () {
        setTimeout(function () {
          line.attr({ opacity: 1 });
          bottomLeft.animate({ points: "199,299 96,299 44,326 199,326" }, 800, mina.bounce);
        }, 100)
      });
    }

    // GSAP Timeline - Zero delay for instant response
    const master = gsap.timeline({ delay: 0 });

    // Helper functions for GSAP
    function circleScale() {
      ...
      tl.fromTo('.circle',
        ...
        .fromTo('.stars',
          ...
          { scale: 1, opacity: 1, duration: 0.8, stagger: 0.05, ease: "elastic.out" },
          'circleIn+=.3'
        );
      return tl;
    }
    ...
    function bodyRotateIn() {
      const tl = gsap.timeline();
      tl.fromTo('#stud_whole',
        { rotation: 90, transformOrigin: "106 260", scale: 0, opacity: 0 },
        { rotation: 0, scale: 1, opacity: 1, duration: 0.6, ease: "power2.out" },
        'bodyRotation'
      )
        .fromTo('#stud_head',
          { rotation: 45, transformOrigin: "85 180" },
          { rotation: 0, duration: 0.5, ease: "back.out" },
          "bodyRotation+=.2"
        )
        .fromTo('#hair',
          { rotation: 15, transformOrigin: "10 50%" },
          { rotation: 0, duration: 1.5, ease: "elastic.out" },
          "bodyRotation+=.25"
        );
      tl.set('#pgFld', { opacity: 1 });
      return tl;
    }

    function repeatAnim() {
      const tl = gsap.timeline({ repeat: -1 });
      tl.to('.eye', { rotationX: 90, transformOrigin: "50% 50%", duration: 1, repeatDelay: 3, repeat: -1, yoyo: true })
        .to('.reflection', { x: 12, opacity: .15, duration: 1.5, repeat: -1, yoyo: true }, 0)
        .to('#dashed', { rotation: 360, transformOrigin: "50% 50%", ease: "none", duration: 20, repeat: -1 }, 0);
      return tl;
    }

    // Strictly parallelize everything for instant result
    master
      .add(() => bookIn(), 0)
      .add(circleScale(), 0)
      .add(bodyRotateIn(), 0)
  ...
  return (
    <div className="student-container">
      <svg id="student" ref={svgRef} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 400 385" style={{ width: '100%', height: 'auto' }}>
        <circle className="circle" style={{ opacity: 0 }} id="BG" opacity="0.2" cx="200.011" cy="208" r="157.657" fill="#FFFFFF" />
        <circle className="circle" style={{ opacity: 0 }} id="dashed" opacity="0.15" fill="none" stroke="#FFFFFF" strokeWidth="1.9028" strokeMiterlimit="10" strokeDasharray="6.5548" cx="200.333" cy="208" r="176.666" />
        <g id="stud_whole" style={{ opacity: 0, shadow: 'none', willChange: 'transform, opacity' }}>
          <g id="stud_body">
          ...
          <g id="stud_head">
            ...
            <g>
              <path fillRule="evenodd" clipRule="evenodd" fill="#6B3815" d="M258,136v30.774c7-0.883,12-5.43,12-10.922V126H258z" />
              <path fillRule="evenodd" clipRule="evenodd" fill="#6B3815" d="M147.109,164.33c-7.143,5.105-18.449,1.699-19.795-9.419 c-10.266-58.371,21.152-60.824,21.152-60.824s-2.678-1.742-4.252-2.767c6.418-3.438,13.241-2.234,19.043,1.522 C187.104,107.034,166.324,151.184,147.109,164.33z" />
              <path fillRule="evenodd" clipRule="evenodd" fill="#4F260B" d="M166.006,96.921l-4.389,10.162 c9.484,31.142-10.533,54.303-10.533,54.303C162.5,151.977,184.316,119.742,166.006,96.921z" />
              <g id="hair">
                <path fillRule="evenodd" clipRule="evenodd" fill="#6B3815" d="M201.883,146.219c1.371,2.254,2.523,4.146,4.095,6.729 c-59.673-18.631-46.111-60.226-41.876-66.788c4.234-6.563,11.658-12.6,20.608-16.324c-0.481,1.902-0.858,3.393-1.235,4.888 c22.131-7.764,49.002-0.148,63.532,10.733c4.941,3.701,9.703,7.613,14.57,11.401c8.224,6.396,17.896,9.472,28.993,10.778 c-5.158,7.781-17.902,14.202-17.902,14.202s7.69,3.34,12.366,5.37C285.034,127.208,254.983,157.615,201.883,146.219z" />
                <path fillRule="evenodd" clipRule="evenodd" fill="#4F260B" d="M256.379,104.706c0,0,16.915,9.361,33.83,3.449 c0,0-4.816,6.628-17.347,14.047C272.862,122.201,260.942,116.496,256.379,104.706z" />
                <path fillRule="evenodd" clipRule="evenodd" fill="#4F260B" d="M193.809,123.123c0,0,23.333,28.178,56.271,22.617 c0,0-27.564,7.207-48.49,0.195C201.59,145.936,193.254,133.809,193.809,123.123z" />
                <path opacity="0.3" fillRule="evenodd" clipRule="evenodd" fill="#4F260B" d="M170.264,132.057 c-4.944,13.301-14.129,26.021-23.009,32.167c0.385-0.181,0.765-0.378,1.132-0.599c8.695-4.995,17.765-15.334,22.678-25.838 c6.094,6.406,16.153,12.604,32.137,17.391c-1.082-1.578-1.949-2.84-2.838-4.135C186.019,145.805,176.47,139.134,170.264,132.057z" />
                <path opacity="0.3" fillRule="evenodd" clipRule="evenodd" fill="#4F260B" d="M250.08,145.74c0,0-25.727,6.724-46.34,0.857 c-0.617-0.123-1.234-0.245-1.857-0.379c0.924,1.518,1.75,2.876,2.67,4.388c38.78,6.563,60.551-8.702,65.84-13.063 c-5.628,3.07-12.816,6.19-21.48,8.359C249.303,145.846,249.688,145.807,250.08,145.74z" />
              </g>
            </g>
            <g id="glasses">
              ...
              <path fill="none" stroke="#000000" strokeWidth="6" strokeMiterlimit="10" d="M208.506,176.158 c-4.578-4.577-11.986-4.577-16.563,0" />
              ...
              <path opacity="0.3" fill="#FFFFFF" d="M257,168.327c0-4.599-3.729-8.327-8.327-8.327h-30.342c-4.6,0-8.329,3.728-8.329,8.327 v19.346c0,4.599,3.729,8.327,8.329,8.327h30.342c4.6,0,8.329-3.728,8.329-8.327V168.327z" />
              <path className="reflection" opacity=".3" fill="#FFFFFF" d="M226.629,160l-13.033,34.105c1.407,1.09,3.168,1.895,5.086,1.895h11.383l13.703-36H226.629z" />
              ...
              <path className="reflection" opacity="0.3" fill="#FFFFFF" d="M160.629,160l-13.033,34.105c1.407,1.09,3.168,1.895,5.086,1.895h11.383l13.703-36H160.629z" />
            </g>
          </g>
        </g>
        ...
        <g id="book" >
          ...
          <path id="bk_edg" fill="none" stroke="#037391" strokeWidth="5" strokeLinecap="round" strokeMiterlimit="10" d="" />
          ...
          <animate
            xlinkHref="#pgFld" attributeName="d" begin="4s" dur="5s" repeatCount="indefinite"
            keyTimes="0; .03; .06; .1; .15; 1"
            values="
